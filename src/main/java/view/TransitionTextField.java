...
public class TransitionTextField extends TextField {
    ...
    private static final Pattern PATTERN_CHARACTER = Pattern.compile("[a-zA-Z0-9/]");
     ...
     */
    public TransitionTextField(String mask) {
        super();
        this.mask = mask;
        ...
        for (int i = 1; i < specialSymbols.toString().length() - 1; i++) {
            expression += "^" + specialSymbols.toString().charAt(i);
        }
        expression += "^" + EMPTY_CHAR + "]";

        PATTERN_ANYTHING = Pattern.compile(expression);
            ...
            if (oldValue != null && newValue != null && !newValue.equals("") &&
                ...
                if (matcher.matches()) {
                    ...
                    if (caretPosition + 1 < text.length() && isSpecial(text.charAt(caretPosition + 1))) {
                        Platform.runLater(() -> positionCaret(text.indexOf("_")==-1?caretPosition+1:text.indexOf("_")));
    ...
    public String textFromMask(Set<Character> specialSymbols) {
    ...
    public char[] getTransition(String input){
        String mask = textFromMask(specialSymbols);
        int i = 4;
        char[] transition = new char[5];
        while (true){
            int index = mask.lastIndexOf("_");
            if(index!=-1) {
                transition[i] = input.charAt(index);
    ...
    public void clearText() {
        text = textFromMask(specialSymbols);
        clear();
    }
